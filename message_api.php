...
// Phase 2 fix: Include profile_helpers.php — needed for profile_pfp_url()
// which is called in the 'get' action to include sender PFPs in the response.
// Without this, the API crashes with "Call to undefined function profile_pfp_url()"
// and returns a 500 error, causing loadChatMessages() to silently fail and
// the main chat area to appear empty.
require_once __DIR__ . '/profile_helpers.php';

// Safety polyfill: if profile_helpers.php didn't load or is an old version,
// define a fallback profile_pfp_url so the API never crashes.
if (!function_exists('profile_pfp_url')) {
  function profile_pfp_url($key = null) {
    $key = strtolower($key ?: '');
    $safe = preg_replace('/[^a-z0-9_-]/i', '-', $key);
    $file = __DIR__ . '/profile_pfps/' . $safe . '.png';
    return file_exists($file) ? '/profile_pfps/' . rawurlencode($safe) . '.png' : '/favicon.png';
  }
}
