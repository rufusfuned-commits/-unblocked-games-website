...
// Phase 3 fix: Use the chat name as the user identity for PFP uploads.
// Previously, this checked profile_account_key() which returns 'guest' for
// all non-admin users (since there's no user login system, only admin login).
// Now we accept a 'name' parameter (sent by the frontend) as the user's
// identity, falling back to the session account_key for admin users.
$key = profile_account_key();
$userName = '';
if ($action === 'upload' && $_SERVER['REQUEST_METHOD'] === 'POST') {
  $userName = trim((string)($_POST['name'] ?? ''));
} elseif ($action === 'status') {
  $userName = trim((string)($_GET['name'] ?? ''));
}
// If we have a chat name from the request, use it as the identity
if ($userName !== '') {
  $key = strtolower($userName);
}
// Only reject if we have NO identity at all (not even a guest)
if ($key === '') {
  echo json_encode(['error' => 'Must provide a name to upload a profile picture.']);
  exit;
}
