...
(function() {
  ...
  async function loadPfpStatus() {
    try {
      // Phase 3 fix: send the chat name so non-admin users can upload too
      const pfpName = chatName || localStorage.getItem('chatName') || '';
      const r = await fetch('/pfp_upload.php?action=status&name=' + encodeURIComponent(pfpName) + '&cache=' + Date.now());
      if (!r.ok) return;
  ...
  if (pfpUploadBtn && pfpInput) {
    pfpUploadBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        fd.append('pfp', file);
        // Phase 3 fix: send the chat name so non-admin users can upload
        const pfpName = chatName || localStorage.getItem('chatName') || '';
        fd.append('name', pfpName);
  ...
  async function loadChatMessages() {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('Chat API returned status:', res.status, 'for URL:', url);
        // Show error in the chat area so the user knows something is wrong
        if (chatMessages && !chatMessages.innerHTML) {
          chatMessages.innerHTML = '<div class="chat-empty">Could not load messages. The chat server may be temporarily unavailable.</div>';
        }
        return;
      }
      const data = await res.json();
